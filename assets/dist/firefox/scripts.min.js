	...
	var fplReadyEvent = {
		...
		observer: function observer() {
			...
			var observer = new MutationObserver(function (mutations) {
				...
				if (mutations.length === 2 && self.emitted === false) return self.emit();

				if (mutations.length === 1 && self.overlay.is(':hidden') && self.emitted === false) return self.emit();
			});
		...
		bindLinks: function bindLinks() {
			...
			this.links = $('a').filter(function (i, link) {
				return $(link).attr('href').indexOf('/a/') > -1;
			});
			...
			this.buttons.on('click', function () {
				_this.emitted = false;console.log(_this.emitted);
			});
		},
		...
		fallBack: function fallBack() {
			...
			var interval = setInterval(function () {
				if (_this2.overlay.is(':hidden') && _this2.emitted === false) _this2.emit();
			}, 1000);

			$(document).on('fplReady', function () {
		...
		init: function init() {
			...
			$(document).on('fplReady', function () {
				return _this3.bindLinks();
			});
		}
	};

	$(document).ready(function () {
		return fplReadyEvent.init();
	});
...
/***/function (module, exports, __webpack_require__) {
	...
	var options = {
		...
		camelCased: function camelCased(string) {
			return string.split('-').map(function (x, i) {
				return i === 0 ? x : x.substr(0, 1).toUpperCase() + x.substr(1);
			}).join('');
		},

		appendTemplate: function appendTemplate() {
			return Promise.resolve($.get(chrome.extension.getURL('/options.html'), function (template) {
				return $($.parseHTML(template)).appendTo('body');
			}));
		},
		...
		updateValues: function updateValues() {
			...
			this.optionValues.each(function (i, element) {
				element = $(element);
				var name = _this4.camelCased(element.attr('id'));
		...
		updateOptionValue: function updateOptionValue(e) {
			var object = {};
			var target = $(e.target);
			var name = this.camelCased(target.attr('id'));
			var value = target.attr('type') === 'checkbox' ? target.is(':checked') : target.val();
		...
		init: function init() {
			...
			this.appendTemplate().then(function () {
				_this5.cache();
				_this5.bind();
				_this5.updateValues();
			});
...
/***/function (module, exports, __webpack_require__) {
	...
	var _watch = __webpack_require__(5).watch;
		...
		sync: function sync(done) {
			...
			chrome.storage.sync.get(function (settings) {
				...
				for (var key in settings) {
					_this6.settings[key] = settings[key];
				}
		...
		onChange: function onChange() {
			...
			chrome.storage.onChanged.addListener(function (changes, namespace) {
				for (var key in changes) {
					_this7.settings[key] = changes[key].newValue;
				}
			});
		},

		watch: function watch(values, callback) {
			_watch(this.settings, values, function (prop, action, newValue, oldValue) {
  ...
  */
	...
	(function (factory) {
		...
		if (isModernBrowser()) {
			...
			defineWatcher = function defineWatcher(obj, prop, watcher) {
			...
			callWatchers = function callWatchers(obj, prop, action, newval, oldval) {
			...
			unwatchOne = function unwatchOne(obj, prop, watcher) {
			...
			callWatchers = function callWatchers(obj, prop, action, value) {
...
/***/function (module, exports, __webpack_require__) {
  ...
  */
	...
	var playerTemplates = {
		...
		init: function init(templateContainers, string) {
	...
	playerTemplates.init(['#ismt-team-summary-item', '#ismt-squad-base-list-summary-item'], 'class="ism-element"');

	playerTemplates.init(['#ismt-statistics-stat-item', '#ismt-squad-selection-list-detail-item', '#ismt-squad-base-elements-list-item', '#ismt-team-my-detail-item'], 'class="ism-table--el__primary"');
...
/***/function (module, exports, __webpack_require__) {
	...
	var playerFixtures = {
		...
		listenOnSettings: function listenOnSettings() {
			var _this9 = this;

			settings.watch('showFixtures', function (newValue) {
				return newValue === true ? _this9.render() : _this9.destroy();
			});
		},

		// Add fixtures to all players in pitch views
		addFixturesToPitchPlayer: function addFixturesToPitchPlayer(elem) {
			if (settings.get('showFixtures') === false) return;
			...
			if (id === undefined) id = elem.find('[data-id]').attr('data-id');

			var player = this.players.find(function (player) {
				return player.id == id;
			});
			var appendTarget = elem.find('.ism-element__controls');
			var fixturesContainer = $('<ul/>', { class: 'fixtures-container--field' });

			// Add the next five fixtures to the fixtures container
			for (var i = 0; i < 5; i++) {
				var fixture = player.fixtures[i];
				var fixtureElement = $('<li/>', {
		...
		addFixturesToListPlayer: function addFixturesToListPlayer(elem) {
			if (settings.get('showFixtures') === false) return;

			elem = $(elem);

			var id = elem.find('.ism-table--el__primary').attr('data-id');
			var player = this.players.find(function (player) {
				return player.id == id;
			});
			var appendTarget = elem.find('.ism-table--el__primary-text .ism-table--el__strong');
			var fixturesContainer = $('<ul/>', { class: 'fixtures-container' });
		...
		watchPitchViews: function watchPitchViews() {
			var _this10 = this;
			...
			var observer = new MutationObserver(function (mutations) {
				...
				if (mutations.length === 19) {
					_this10.pitchPlayers = _this10.pitchViews.find('.ism-element');
					return _this10.pitchPlayers.each(function (i, elem) {
						return _this10.addFixturesToPitchPlayer(elem);
					});
				}

				mutations.forEach(function (mutation) {
					if (mutation.addedNodes.length > 0 && mutation.addedNodes[0].className !== 'fixtures-container--field') _this10.addFixturesToPitchPlayer(mutation.addedNodes[0]);
				});
			});

			// Observe each pitch view
			this.pitchViews.each(function (i, view) {
				return observer.observe(view, { subtree: true, childList: true });
			});
		},

		// Apply a mutation observer on the list views
		watchListViews: function watchListViews() {
			var _this11 = this;
			...
			var observer = new MutationObserver(function (mutations) {
				...
				if (mutations.length !== _this11.listPlayers.length) {
					_this11.listPlayers = _this11.listViews.find('.ism-row-select');
					_this11.listPlayers.each(function (i, elem) {
						return _this11.addFixturesToListPlayer(elem);
					});
				}
			});

			this.listViews.each(function (i, view) {
				return observer.observe(view, { subtree: true, childList: true });
			});
		},
		...
		render: function render() {
			...
			this.pitchPlayers.each(function (i, elem) {
				return _this12.addFixturesToPitchPlayer(elem);
			});
			this.listPlayers.each(function (i, elem) {
				return _this12.addFixturesToListPlayer(elem);
			});
		},
...
/***/function (module, exports, __webpack_require__) {
	...
	var elements = {
		...
		loadElementSummary: function loadElementSummary(id, team) {
			return Promise.resolve($.getJSON('https://fantasy.premierleague.com/drf/element-summary/' + id, function (data) {
				data.team = team;
				return data;
			}));
		},

		loadWatchList: function loadWatchList() {
		...
		get: function get() {
			...
			if (this.elements.length !== 0) return Promise.resolve(this.elements);

			var elements = this.loadElements().then(function (data) {
				_this14.elements = data.elements;

				var _loop = function _loop() {
					var team = _this14.teams[i];

					if (done.indexOf(team) < 0) {
						var id = _this14.elements.find(function (element) {
							return element.team === team;
						}).id;
						promises.push(_this14.loadElementSummary(id, team));
					}
				};

				for (var i = 0; i < _this14.teams.length; i++) {
					_loop();
				}

				return Promise.all(promises);
			}).then(function (data) {
				_this14.elements = _this14.elements.map(function (element) {
					element.fixtures = data.find(function (d) {
						return d.team === element.team;
					}).fixtures;
					return element;
				});

				return _this14.elements;
			}).catch(function (err) {
				throw err;
			});
		...
		getWatchlistPlayers: function getWatchlistPlayers() {
...
/***/function (module, exports, __webpack_require__) {
	...
	var transferModal = {
		...
		doListElementAction: function doListElementAction() {
			var _this16 = this;

			if (settings.get('hideModals') === false) return;

			this.modal.hide();

			// We have to make sure this function runs synchronously beacuse of react
			setTimeout(function () {
				_this16.updateButtons();

				if (_this16.playerButtonAdd.length === 0 || _this16.playerButtonAdd.is(':disabled')) _this16.playerButtonInfo.click();else _this16.playerButtonAdd.click();
			});
		},
...
/***/function (module, exports, __webpack_require__) {
	...
	var playerDialogs = {
		...
		listenOnSettings: function listenOnSettings() {
			...
			settings.watch('disableModalHorizontalScroll', function (newValue) {
				return newValue === true ? _this17.addClass() : _this17.removeClass();
			});
		},

		addClass: function addClass() {
			if (settings.get('disableModalHorizontalScroll') === true) this.body.addClass('dialog-max-width');
		},

		removeClass: function removeClass() {
			if (settings.get('disableModalHorizontalScroll') === false) this.body.removeClass('dialog-max-width');
		},
		...
		init: function init() {
			...
			if (settings.get('disableModalHorizontalScroll') === true) this.addClass();else this.removeClass();
		}

	};

	$(document).on('fplReady', function () {
		return playerDialogs.init();
	});
...
/***/function (module, exports, __webpack_require__) {
	...
	var playerHighlights = {
		...
		listenOnSettings: function listenOnSettings() {
			...
			settings.watch('transferHighlight', function (newValue) {
				return newValue === true ? _this18.render() : _this18.destroy();
			});
		},

		// Add fixtures to a player in list view
		addHighlighttoListRow: function addHighlighttoListRow(elem) {
			if (!settings.get('transferHighlight')) return;

			elem = $(elem);
			var code = elem.attr('data-code');

			if (code === undefined) code = elem.find('[data-code]').attr('data-code');

			if (elem.hasClass("ism-element-list__in-squad")) {
				elem.css('background-color', 'lightcoral');

				// Fix text color for child elements
				var appendTarget = elem.find('.ism-table--el__strong');
		...
		watchListViews: function watchListViews() {
			...
			var observer = new MutationObserver(function (mutations) {
				...
				if (mutations.length !== _this19.listPlayers.length) {
					_this19.listPlayers = _this19.listViews.find('.ism-row-select');
					_this19.listPlayers.each(function (i, elem) {
						return _this19.addHighlighttoListRow(elem);
					});
					players.getWatchlistPlayers().then(function (players) {
						_this19.players = players;
						_this19.render();
					});
				}
			});
		...
		render: function render() {
			...
			this.listPlayers.each(function (i, elem) {
				return _this20.addHighlighttoListRow(elem);
			});
		},
		...
		destroy: function destroy() {
			this.listPlayers.each(function (i, elem) {
				return $(elem).css('background-color', 'white');
			});
		},
